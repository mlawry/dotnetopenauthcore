...
namespace DotNetOpenAuth.Messaging {
    ...
    internal class MessageSerializer {
        ...
        /// <summary>
        /// Reads JSON as a flat dictionary into a message. Uses json.net.
        /// </summary>
        /// <param name="messageDictionary">The message dictionary to fill with the JSON-deserialized data.</param>
        /// <param name="jsonStr">The JSON string.</param>
        internal static void DeserializeJsonAsFlatDictionary(IDictionary<string, string> messageDictionary, string jsonStr)
        {
            Requires.NotNull(messageDictionary, "messageDictionary");
            Requires.NotNull(jsonStr, "jsonStr");

            JObject jobj = JObject.Parse(jsonStr);

            foreach (JProperty jprop in jobj.Properties())
            {
                messageDictionary[jprop.Name] = jprop.Value.Value<string>();
            }
        }
