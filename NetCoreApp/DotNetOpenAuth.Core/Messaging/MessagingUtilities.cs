...
namespace DotNetOpenAuth.Messaging
{
	...
	public static class MessagingUtilities {
		...
		internal static readonly RandomNumberGenerator CryptoRandomDataGenerator = RandomNumberGenerator.Create();
		...
		/// </summary>
		private static readonly TimeSpan SymmetricSecretKeyLifespan = DotNetOpenAuthConfiguration.MessagingElement.PrivateSecretMaximumAge;
		...
		public static Uri StripQueryArgumentsWithPrefix(this Uri uri, string prefix) {
			...
			Dictionary<string, StringValues> queryArgs = QueryHelpers.ParseQuery(uri.Query);
		...
		/// </remarks>
		public static Uri GetPublicFacingUrl(this HttpRequest request, Dictionary<string, StringValues> serverVariables) {
			...
			if (httpHost != null) {
				ErrorUtilities.VerifySupported(request.Scheme == "http" || request.Scheme == "http", "Only HTTP and HTTPS are supported protocols.");
                StringValues forProto = serverVariables["HTTP_X_FORWARDED_PROTO"];
                if (forProto == (object)null || forProto.Count == 0)
                {
                    if (string.Equals(serverVariables["HTTP_FRONT_END_HTTPS"], "on", StringComparison.OrdinalIgnoreCase))
                    {
                        forProto = "https";
                    }
                    else
                    {
                        forProto = request.Scheme;
                    }
                }
                string scheme = forProto[0];

				Uri hostAndPort = new Uri(scheme + "://" + request.Host);
				UriBuilder publicRequestUri = new UriBuilder();
				publicRequestUri.Scheme = scheme;
				publicRequestUri.Host = hostAndPort.Host;
				publicRequestUri.Port = hostAndPort.Port; // CC missing Uri.Port contract that's on UriBuilder.Port
                publicRequestUri.Path = request.Path;
                publicRequestUri.Query = request.QueryString.ToString();
                ...
                throw new NotSupportedException("Cannot work out a way to obtain the public facing URL for the given request.");
			}
		}
		...
		public static Uri GetPublicFacingUrl(this HttpRequest request) {
			Requires.NotNull(request, "request");
			return GetPublicFacingUrl(request, new Dictionary<string, StringValues>());
		}
		...
		//	var httpRequest = new HttpRequestMessage(new HttpMethod(request.Method), publicFacingUrl);
		...
		/// </returns>
		public static async Task SendAsync(this HttpResponseMessage response, HttpContext context, CancellationToken cancellationToken = default(CancellationToken)) {
			...
			foreach (var header in response.Headers) {
                StringValues headerValues = new StringValues(header.Value.ToArray());
				responseContext.Headers[header.Key] = headerValues;
			}

			if (response.Content != null) {
				await response.Content.CopyToAsync(responseContext.Body).ConfigureAwait(false);
			}
		}
		...
		internal static byte[] EncryptWithRandomSymmetricKey(this RSACryptoServiceProvider crypto, byte[] buffer) {
            ...
            // YTML: There is no SymmetricAlgorithm.Create() method, but it looks like you can choose an algorithm directly
            // by creating an instance of that class. I'm choosing Aes because there looks to be only 2 choices (checked via
            // Object Browser) Aes and TripleDES, and it looks like Aes is stronger than TripleDES.
            using (var symmetricCrypto = Aes.Create()) {
		...
		internal static void CopyHeadersFrom(this HttpRequestMessage message, HttpRequest request) {
			Requires.NotNull(request, "request");
			Requires.NotNull(message, "message");

			foreach (KeyValuePair<string, StringValues> kvPair in request.Headers) {
                string headerName = kvPair.Key;
                string[] headerValues = kvPair.Value;
		...
		internal static void AppendAndReplaceQueryArgs(this UriBuilder builder, IEnumerable<KeyValuePair<string, string>> args) {
			...
			if (args != null && args.Count() > 0) {
				Dictionary<string, StringValues> aggregatedArgs = QueryHelpers.ParseQuery(builder.Query);
		...
		public static IEnumerable<KeyValuePair<string, string>> AsKeyValuePairs(this Dictionary<string, StringValues> nvc) {
			Requires.NotNull(nvc, "nvc");

			foreach (KeyValuePair<string, StringValues> kvPair in nvc) {
				foreach (string value in kvPair.Value) {
					yield return new KeyValuePair<string, string>(kvPair.Key, value);
				}
			}
		}
		...
		/// </remarks>
		public static Dictionary<string, string> ToDictionary(this Dictionary<string, StringValues> nvc) {
		...
		internal static Dictionary<string, string> ToDictionary(this Dictionary<string, StringValues> nvc, bool throwOnNullKey) {
			if (nvc == null) {
				return null;
			}
			...
			foreach (KeyValuePair<string, StringValues> kvPair in nvc) {
                ...
                // YTML: keeping the code here even though it is redundant since NetCoreApp uses Dictionary which does not allow nulls.
				if (kvPair.Key == null) {
					if (throwOnNullKey) {
						...
						if (!string.IsNullOrEmpty(kvPair.Value)) {
							Logger.OpenId.WarnFormat("Null key with value {0} encountered while translating NameValueCollection to Dictionary.", kvPair.Value);
						}
					}
				} else {
                    // YTML: Joins the string together using a comma. Source code in repo https://github.com/aspnet/Common/
                    dictionary.Add(kvPair.Key, kvPair.Value.ToString());
				}
			}
		...
		internal static Dictionary<string, StringValues> ToNameValueCollection(this IDictionary<string, string> data) {
			Requires.NotNull(data, "data");
        ...
        private static Dictionary<string, StringValues> ToNameValueCollection(IEnumerable<KeyValuePair<string, StringValues>> kvEnumeration)
        {
            if (kvEnumeration == null)
            {
                return null;
            }
            var dict = new Dictionary<string, StringValues>(kvEnumeration.Count());
            foreach (KeyValuePair<string, StringValues> kvPair in kvEnumeration)
            {
		...
		internal static string EscapeUriDataStringRfc3986(string value) {
			...
			for (int i = 0; i < UriRfc3986CharsToEscape.Length; i++) {
                char letter = UriRfc3986CharsToEscape[i][0];               // Convert the character to hexadecimal string.
                int letterValue = Convert.ToInt32(letter);                 // Get the integral value of the character.
                string letterHexStr = string.Format("{0:X}", letterValue); // Convert the decimal value to a hexadecimal value in string form.
                escaped.Replace(UriRfc3986CharsToEscape[i], letterHexStr);
			}
		...
		/// </returns>
		internal static Dictionary<string, StringValues> GetQueryStringBeforeRewriting(this HttpRequest request) {
			...
			if (string.Equals(beforeRewriting.AbsolutePath, request.Path, StringComparison.Ordinal) &&
                string.Equals(beforeRewriting.Query, request.QueryString.ToString(), StringComparison.Ordinal)) {
                // No rewriting has taken place.
                return ToNameValueCollection(request.Query);
			} else {
				...
				return QueryHelpers.ParseQuery(beforeRewriting.Query);
			}
		}
		...
		/// </value>
		internal static bool GetIsUrlRewritten(this HttpRequest request) {
		...
		internal static Dictionary<string, StringValues> GetQueryOrForm(this HttpRequest request) {
			Requires.NotNull(request, "request");
            if ("GET".Equals(request.Method, StringComparison.OrdinalIgnoreCase))
            {
                return GetQueryStringBeforeRewriting(request);
            }
            else
            {
                return ToNameValueCollection(request.Form);
            }
		}
