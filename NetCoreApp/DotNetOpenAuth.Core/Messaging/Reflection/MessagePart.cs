...
namespace DotNetOpenAuth.Messaging.Reflection {
	...
	internal class MessagePart {
        ...
        /// <summary>
        /// Extracted from SetValue() method, which previously read from MessageElement directly.
        /// </summary>
        internal bool IsValueCaseSensitive { get; set; } = DotNetOpenAuthConfiguration.MessagingElement.Strict;
        ...
        internal void SetValue(IMessage message, string value) {
			...
			try {
				if (this.IsConstantValue) {
					...
					var caseSensitivity = this.IsValueCaseSensitive ? StringComparison.Ordinal : StringComparison.OrdinalIgnoreCase;
		...
		internal bool IsNondefaultValueSet(IMessage message) {
			if (this.memberDeclaredType.GetTypeInfo().IsValueType) {
		...
		private static ValueMapping GetDefaultEncoder(Type type) {
			...
			var converterAttributes = (DefaultEncoderAttribute[])type.GetTypeInfo().GetCustomAttributes(typeof(DefaultEncoderAttribute), false);
		...
		private void ValidateSettings() {
			// YTML: commented out because an encoder can convert certain values of the non-nullable type into null,
			// so a non-nullable type can still be optional by using this technique.
