...
namespace DotNetOpenAuth.OAuth2 {
	...
	public static class OAuthUtilities {
		...
		public static void AuthorizeWithBearerToken(this HttpWebRequest request, string accessToken) {
        ...
        private static byte[] Section231Encode(string value)
        {
            byte[] encValue = HttpBasicEncoding.GetBytes(value);
            byte[] urlEncValue = WebUtility.UrlEncodeToBytes(encValue, 0, encValue.Length);
            return urlEncValue;
        }

        private static string Section231Decode(byte[] array)
        {
            byte[] encValue = WebUtility.UrlDecodeToBytes(array, 0, array.Length);
            return HttpBasicEncoding.GetString(encValue);
        }
	}
}
