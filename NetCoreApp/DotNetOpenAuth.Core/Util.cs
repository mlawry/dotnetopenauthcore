...
namespace DotNetOpenAuth {
	...
	internal static class Util {
		...
		private static readonly Lazy<ProductInfoHeaderValue> libraryVersionHeaderLazy = new Lazy<ProductInfoHeaderValue>(delegate {
			var assemblyName = typeof(Util).GetTypeInfo().Assembly.GetName();
		...
		/// </summary>
		private static IEmbeddedResourceRetrieval embeddedResourceRetrieval = DotNetOpenAuthConfiguration.MessagingElement?.EmbeddedResourceRetrievalProvider?.CreateInstance(null, false, null);
		...
		internal static string AssemblyFileVersion {
			get {
				var assembly = typeof(Util).GetTypeInfo().Assembly;
				var attributes = assembly.GetCustomAttributes(typeof(AssemblyFileVersionAttribute));
				if (attributes.Count() == 1) {
					var fileVersionAttribute = (AssemblyFileVersionAttribute)attributes.First();
