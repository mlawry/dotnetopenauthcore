...
namespace DotNetOpenAuth.OAuth2
{
    ...
    public class ClientBase
    {
        ...
        /// <summary>
        /// Sends a JSON Web Token assertion and receives the access token.
        /// </summary>
        /// <param name="scopes">The desired scopes.</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        /// <returns>
        /// The result of the authorization request.
        /// </returns>
        public Task<IAuthorizationState> GetAssertionAccessTokenAsync(IEnumerable<string> scopes = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            // YTML: Assume we've been given an assertion provider via the constructor, stored as a ClientCredentialApplicator.
            var assertProvApp = this.ClientCredentialApplicator as ClientCredentialApplicator.AssertionProviderApplicator;
            if (assertProvApp == null)
            {
                throw new InvalidOperationException("To use this method, you must instantiate this class with an IAssertionProvider.");
            }

            var request = new AccessTokenAssertionRequest(assertProvApp.AssertionProvider, this.AuthorizationServer.TokenEndpoint, this.AuthorizationServer.Version);
            return this.RequestAccessTokenAsync(request, scopes, cancellationToken);
        }
        ...
        protected void ApplyClientCredential(IAccessTokenRequest request)
        {
            Requires.NotNull(request, "request");
